...
import yaml
with open("/mnt/home/spandey/ceph/AR_NPE/run_scripts/run_test_128.yaml","r") as file_object:
...
class LoadCustomData(Dataset):
    def __init__(self, return_dict, return_dict_FP, all_gpu=False):
        ...
        # mask_tensor_Mdiff_train_FP = torch.Tensor((np.array(return_dict_train_FP['mask_M_diff'])[batch_here,...].reshape(nbatches_train_jt,nsims_per_batch * (nax_h**3),
                                                                                        # ndim_diff))).cuda(dev)
    ...
    def __getitem__(self, idx):
        ...
        return (cond_tensor, cond_tensor_nsh, mask_tensor_M1_train, mask_tensor_Mdiff_train, X_M1, X_Nhalo, X_Mdiff, Nhalos_truth_tensor, mask_tensor_M1_train_FP, X_M1_FP)
    ...
    if nepochs > 0:
        ...
        for jt in tqdm(range(nepochs)):
            for jd in range(len(dataloader)):
                torch.cuda.empty_cache()
                optimizer.zero_grad()
                cond_tensor_jd, cond_tensor_nsh_jd, mask_tensor_M1_train_jd, mask_tensor_Mdiff_train_jd, X_M1_jd, X_Nhalo_jd, X_Mdiff_jd, Nhalos_truth_tensor_jd, mask_tensor_M1_train_FP_jd, X_M1_FP_jd = next(iter(dataloader))
                torch.cuda.empty_cache()
                if 1-all_gpu:
            ...
            if (np.mod(jt, int(nepochs / 25)) == 0) or (jt == nepochs - 1):
                if float(loss.cpu().detach().numpy()) < loss_min:
                    ...
                    print(loss_min, lr)
                    state = {'loss_min': loss_min, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict(), 'scheduler': scheduler.state_dict(), 'loss':loss, 'lr':lr}
