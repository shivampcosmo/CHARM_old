...
import pickle as pk
df = pk.load(open('/mnt/home/spandey/ceph/AR_NPE/notebooks/COSMO_VARY_RUNS_128/' + 'VARY_COSMO_SUBSEL_random_nsims512_nspji8_nfid512' + '_train_data_QUIJOTE.pk', 'rb'))

df_d_all_train = df['df_d_all_train']
df_d_all_nsh_train = df['df_d_all_nsh_train']
df_Mh_all_train = df['df_Mh_all_train']
df_Nh_train = df['df_Nh_train']
...
import pickle as pk
df = pk.load(open('/mnt/home/spandey/ceph/AR_NPE/notebooks/COSMO_VARY_RUNS_128/' + 'VARY_COSMO_SUBSEL_random_nsims512_nspji8_nfid512' + '_train_data_FASTPM.pk', 'rb'))
...
return_dict_train_FP = prep_density_halo_cats_batched(
    df_d_all_train_FP, df_d_all_nsh_train_FP, df_Mh_all_train_FP, df_Nh_train,cosmo_val_all_train_FP, nsims=nsims_per_batch,
...
cond_cosmo = cond_cosmo.cuda(dev)
...
binary_mask_tensor = torch.zeros_like(Nhalos_truth_tensor)
ind_gt = torch.where(Nhalos_truth_tensor > 0)
binary_mask_tensor[ind_gt] = 1
X_binary_int = binary_mask_tensor.to(dev)
X_binary = return_dict_train['sig_all'][0] * torch.randn(X_binary_int.shape[0], X_binary_int.shape[1], X_binary_int.shape[2], device=dev) + (X_binary_int) + 1

X_multiclass_int = ((Nhalos_truth_tensor).to(torch.long)).to(dev)
X_multiclass = return_dict_train['sig_all'][0] * torch.randn(X_multiclass_int.shape[0], X_multiclass_int.shape[1], X_multiclass_int.shape[2], device=dev) + (X_multiclass_int)
...
save_bestfit_model_dir = '/mnt/home/spandey/ceph/AR_NPE/' + \
                        'TEST_VARY_COSMO/HRES_SUMGAUSS_subsel_random_LONGTRAIN_Ntot_only_plexp_ns_' + \
                            str(len(ji_array)) + \
...
nepochs_all = [3000, 10000, 20000]

# train_binary_all = [1, 1]
# train_multi_all = [0, 1]
# train_M1_all = [0,0]
# train_Mdiff_all = [0,0]
# nepochs_all = [5000,15000]
...
for js in (range(len(train_binary_all))):
    ...
    for jt in tqdm(range(nepochs)):
        optimizer.zero_grad()
        cond_tensor_jd, cond_tensor_nsh_jd, cond_cosmo_jd, mask_tensor_M1_train_jd, mask_tensor_Mdiff_train_jd, X_M1_jd, \
            X_Nhalo_jd, X_Mdiff_jd, Nhalos_truth_tensor_jd, mask_tensor_M1_train_FP_jd, X_M1_FP_jd = cond_tensor, cond_tensor_nsh, cond_cosmo, \
        mask_tensor_M1_train, mask_tensor_Mdiff_train, X_M1, \
        ...
        loss = model(
            ...
            cond_cosmo=cond_cosmo_jd,
            indsel_Nhalo_gt1=indsel_Nhalo_gt1,
        ...
        if (np.mod(jt, int(nepochs / 150)) == 0) or (jt == nepochs - 1):
            if float(loss.cpu().detach().numpy()) < loss_min:
                loss_min = float(loss.cpu().detach().numpy())
                print(loss_min)

                state = {'loss_min': loss_min, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict(),
                        'scheduler': scheduler.state_dict(), 'loss':loss, 'epoch_tot_counter':epoch_tot_counter}
